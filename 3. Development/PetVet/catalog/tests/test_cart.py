...
class CartWithProductTest(TestCase):
    ...
    def create_product(self, name, slug, price=Decimal("100"), stock=5, 
        ...
        product = Product.objects.create(category=self.category, name=name, slug=slug, description='', price=price, available=True, stock=stock, created_at=created_at)
...
class CartViewTest(TestCase):
    ...
    def test_cart_detail_view_get(self):
        response = self.client.get(reverse('cart:cart_add', kwargs={'product_id':self.product.id}))
        self.assertEqual(response.status_code, 200)
        #fix me, path might be incorrect
        self.assertTemplateUsed(response,'cart/detail.html')
        self.failUnless(isinstance(response.context['cart'], Cart))
